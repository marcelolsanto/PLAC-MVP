  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        ...
        const res = await api.get(/catmat-search/?q=&tipo=);
  ...
  return (
    <div ref={wrapperRef} className='relative'>
      <input
        ...
        className={w-full bg-slate-800 border  text-slate-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500}
        ...
        <div className='absolute z-10 w-full mt-1 bg-slate-800 border border-slate-700 rounded shadow-lg max-h-60 overflow-auto'>
          {results.map((item, idx) => (
            <div
              key={idx}
              onClick={() => {
                const val = ${item.tipo === 'M' ? 'CATMAT' : 'CATSER'}  - ;
                setQuery(val);
                onChange(val);
                setIsOpen(false);
              }}
